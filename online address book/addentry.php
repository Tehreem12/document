...
include 'include.php';
 ...
 $post_names_sql = "SELECT concat_ws(' ', f_name, l_name) AS
display_name
 FROM master_name WHERE id = '".$safe_id."'";
 $post_names_res = mysqli_query($mysqli, $post_names_sql)
 or die(mysqli_error($mysqli));
 if (mysqli_num_rows($post_names_res) == 1) {
 while ($name_info = mysqli_fetch_array($post_names_res)) {
...
<fieldset>
 ...
 <input type="radio" id="add_type_w" name="add_type"
 value="work" />
 <label for="add_type_w">work</label>
 <input type="radio" id="add_type_o" name="add_type"
 value="other" />
 ...
 <input type="radio" id="tel_type_h" name="tel_type"
 value="home" checked />
 <label for="tel_type_h">home</label>
 <input type="radio" id="tel_type_w" name="tel_type"
 value="work" />
 <label for="tel_type_w">work</label>
 <input type="radio" id="tel_type_o" name="tel_type"
 value="other" />
...
<legend>Fax Number:</legend><br/>
 ...
 <input type="radio" id="fax_type_h" name="fax_type"
 value="home" checked />
<label for="fax_type_h">home</label>
 <input type="radio" id="fax_type_w" name="fax_type"
 value="work" />
 ...
 <input type="radio" id="email_type_h" name="email_type"
 value="home" checked />
 <label for="email_type_h">home</label>
 <input type="radio" id="email_type_w" name="email_type"
 value="work" />
 <label for="email_type_w">work</label>

 <input type="radio" id="email_type_o" name="email_type"
value="other" />
...
END_OF_TEXT;
 ...
 zipcode, type) VALUES ('".$master_id."', now(), now(),'".$safe_address."', '".$safe_city."','".$safe_state."' , '".$safe_zipcode."' ,
'".$_POST['add_type']."')";
 ...
 $add_tel_sql = "INSERT INTO telephone (master_id, date_added,date_modified, tel_number, type) VALUES
 ('".$master_id."', now(), now(),
 ...
 '".$safe_note."', date_modified = now()

 WHERE master_id = '".$master_id."'";
 }
 mysqli_close($mysqli);
 $display_block = "<p>Your entry has been added. Would you
